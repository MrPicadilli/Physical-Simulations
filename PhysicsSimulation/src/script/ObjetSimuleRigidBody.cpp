...
void ObjetSimuleRigidBody::initMeshObjet()
{
    ...
    std::cout << "je suis : " << _name << "ma taille est de :" << P.size() << std::endl;
    if (this->_use_texture)
    {
        m_ObjetSimule = Mesh(GL_TRIANGLES);
        for (unsigned int i = 0; i < P.size(); i++)
        {
            m_ObjetSimule.texcoord(_vectTexture.at(i).u, _vectTexture.at(i).v);
            m_ObjetSimule.vertex(Point(P.at(i)));
            m_ObjetSimule.normal(-_vectNormals[i]); // les normales sont soi diant pas dans le bon sens
        }

        for (unsigned int i = 0; i < _VISize; i += 3)
        {
            m_ObjetSimule.triangle(_VIndices.at(i), _VIndices.at(i + 1), _VIndices.at(i + 2));
        }
    }
    else
    {
...
void ObjetSimuleRigidBody::updateVertex()
{
    ...
    for (unsigned int i = 0; i < P.size(); i++)
    {
        m_ObjetSimule.vertex(i, Point(P.at(i)));
        //m_ObjetSimule.normal(i, _Rotation * _vectNormals[i] + _BaryCentre);
